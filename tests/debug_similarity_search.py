...
def main():
    ...
    parser.add_argument("--query", default="có bao nhiêu chi nhánh?", help="Câu hỏi cần kiểm tra")
    ...
    def run_once(label: str, query: str):
        ...
        for ns in namespaces:
            ...
            for i, item in enumerate(results, 1):
                ...
                if has_branch:
                    branch_hits += 1
                print(f"    {i:02d}. score={score:.4f} id={key} {'✅' if has_branch else '  '} :: {snippet}")
            print(f"  ➜ Tổng văn bản có tín hiệu chi nhánh/địa chỉ: {branch_hits}/{len(results)}\n")
    ...
    print("Hoàn tất. Dựa trên tỷ lệ kết quả có tín hiệu địa chỉ/chi nhánh, hãy chỉnh namespace (LOCATION_NAMESPACE) hoặc bổ sung dữ liệu.")
