...
class QueryClassifier:
    ...
    def classify_query(self, query: str) -> Dict[str, any]:
        ...
        Returns:
            Dictionary containing classification results and retrieval parameters
        """
        if not query:
        ...
        primary_category = self._get_primary_category(is_menu_query, is_location_query, is_promotion_query)
        
        # Determine retrieval limit
        retrieval_limit = 12 if (is_menu_query or is_location_query or is_promotion_query) else 5
    ...
    def _get_primary_category(self, is_menu: bool, is_location: bool, is_promotion: bool) -> str:
    ...
    def _get_expansion_keywords(self, category: str) -> List[str]:
        ...
        expansion_map = {
            ...
            "location": ["địa chỉ", "chi nhánh", "branch", "Hotline", "Hà Nội", "Hải Phòng", "TP. Hồ Chí Minh", "Times City", "Vincom", "Tian Long"],
            "promotion": ["ưu đãi", "khuyến mãi", "giảm giá", "chương trình thành viên", "thẻ thành viên", "BẠC", "VÀNG", "KIM CƯƠNG", "sinh nhật", "Tian Long"],
            "general": ["Tian Long"]
        }
...
def generate_dynamic_prompt_sections(classification: Dict[str, any]) -> Dict[str, str]:
    ...
    relevance_boost = ""
    if category == "menu":
        relevance_boost = f"RELEVANCE BOOST FOR MENU QUERIES: Documents containing menu signals are highly relevant. Menu signals: {', '.join(signals[:10])}..."
    elif category == "location":
        relevance_boost = f"RELEVANCE BOOST FOR LOCATION QUERIES: Documents containing location signals are highly relevant. Location signals: {', '.join(signals[:10])}..."
    elif category == "promotion":
        relevance_boost = f"RELEVANCE BOOST FOR PROMOTION QUERIES: Documents containing promotion signals are highly relevant. Promotion signals: {', '.join(signals[:10])}..."
    
    # Generate rewrite instructions
    expansion_keywords = classification["expansion_keywords"]
    rewrite_instruction = f"For this query type, consider appending these relevant keywords: {', '.join(expansion_keywords[:8])}..."
